# -*- coding: utf-8 -*-
"""
Created on Thu Feb 21 14:41:38 2019
...
def loss(sess, model):
       ...
       for img in model.style:
              # 将风格图片作为输入，方便后面提取风格图片在各层中的特征矩阵
              sess.run(tf.assign(model.net['input'],img))
       ...
       # 将损失对层数取平均
              ...
              total_style_loss+=style_loss*1
       
       total_style_loss /= float(len(model.style))
       # 将内容损失和风格损失加权求和，构成总损失函数
       loss = settings.ALPHA * content_loss + settings.BETA * total_style_loss
...
def train():
       # 创建一个模型
       model = model1.Model(settings.CONTENT_IMAGE, settings.STYLE_IMAGE)
       # 创建session
       with tf.Session() as sess:
              ...
              for step in range(settings.TRAIN_STEPS):
                     ...
                     # 取出input的内容，这是生成的图片
                            ...
                            # 保存图片
                            scipy.misc.imsave('./example2/{}-{}.jpg'.format(settings.OUTPUT_IMAGE,step), img)
                     # 保存最终训练结果
              img = sess.run(model.net['input'])
              img += settings.IMAGE_MEAN_VALUE
              img = img[0]
              img = np.clip(img, 0, 255).astype(np.uint8)
              scipy.misc.imsave('./example2/{}.jpg'.format(settings.OUTPUT_IMAGE), img)

if __name__ == '__main__':
       '''
       def numInput(numList):
              path = input("please input a num:" )
              numList.append(path)
              numInput(numList)
              if path == 'done':
                     ...
                     return True
       '''
       #path=input(u"请输入风格图片路径:")
       #setting.STYLE_IMAGE.append(path)
       #weight=input(u"请输入风格图片权重:")
       #setting.imgs_weights.append(weight)
       train()